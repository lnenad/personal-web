...
const stmtRetrievePostsForIndex = "SELECT id, uuid, title, slug, tagline, markdown, html, featured, page, status, meta_description, image, author_id, published_at FROM posts WHERE page = 0 AND status = 'published' ORDER BY published_at DESC LIMIT ? OFFSET ?"
const stmtRetrievePostsForApi = "SELECT id, uuid, title, slug, tagline, markdown, html, featured, page, status, meta_description, image, author_id, published_at FROM posts ORDER BY id DESC LIMIT ? OFFSET ?"
const stmtRetrievePostsByUser = "SELECT id, uuid, title, slug, tagline, markdown, html, featured, page, status, meta_description, image, author_id, published_at FROM posts WHERE page = 0 AND status = 'published' AND author_id = ? ORDER BY published_at DESC LIMIT ? OFFSET ?"
const stmtRetrievePostsByTag = "SELECT posts.id, posts.uuid, posts.title, posts.slug, posts.tagline, posts.markdown, posts.html, posts.featured, posts.page, posts.status, posts.meta_description, posts.image, posts.author_id, posts.published_at FROM posts, posts_tags WHERE posts_tags.post_id = posts.id AND posts_tags.tag_id = ? AND page = 0 AND status = 'published' ORDER BY posts.published_at DESC LIMIT ? OFFSET ?"
const stmtRetrievePostById = "SELECT id, uuid, title, slug, tagline, markdown, html, featured, page, status, meta_description, image, author_id, published_at FROM posts WHERE id = ?"
const stmtRetrievePostBySlug = "SELECT id, uuid, title, slug, tagline, markdown, html, featured, page, status, meta_description, image, author_id, published_at FROM posts WHERE slug = ?"
...
func RetrievePostsForIndex(limit int64, offset int64) ([]structure.Post, error) {
	...
	if err != nil {
		fmt.Println("Error getting posts for index: ", err)
		return nil, err
	}
	defer rows.Close()
	posts, err := extractPosts(rows)
...
func RetrievePostsForApi(limit int64, offset int64) ([]structure.Post, error) {
	...
	if err != nil {
		fmt.Println("Error retreiving posts:", err)
		return nil, err
	}
	defer rows.Close()
	posts, err := extractPosts(rows)
...
func extractPosts(rows *sql.Rows) (*[]structure.Post, error) {
	...
	for rows.Next() {
		...
		var userId int64
		var status string
		err := rows.Scan(&post.Id, &post.Uuid, &post.Title, &post.Slug, &post.Tagline, &post.Markdown, &post.Html, &post.IsFeatured, &post.IsPage, &status, &post.MetaDescription, &post.Image, &userId, &post.Date)
		if err != nil {
			fmt.Println("Error extracting posts: ", err)
			return nil, err
		}
...
func extractPost(row *sql.Row) (*structure.Post, error) {
	...
	var userId int64
	var status string
	err := row.Scan(&post.Id, &post.Uuid, &post.Title, &post.Slug, &post.Tagline, &post.Markdown, &post.Html, &post.IsFeatured, &post.IsPage, &status, &post.MetaDescription, &post.Image, &userId, &post.Date)
	if err != nil {
		return nil, err
	}
	// If there was no publication date attached to the post, make its creation date the date of the post
