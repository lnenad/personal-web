...
const stmtUpdatePost = "UPDATE posts SET title = ?, slug = ?, tagline = ?, markdown = ?, html = ?, featured = ?, page = ?, status = ?, meta_description = ?, image = ?, updated_at = ?, updated_by = ? WHERE id = ?"
const stmtUpdatePostPublished = "UPDATE posts SET title = ?, slug = ?, tagline = ?, markdown = ?, html = ?, featured = ?, page = ?, status = ?, meta_description = ?, image = ?, updated_at = ?, updated_by = ?, published_at = ?, published_by = ? WHERE id = ?"
...
func UpdatePost(id int64, title []byte, slug string, tagline string, markdown []byte, html []byte, featured bool, isPage bool, published bool, meta_description []byte, image []byte, updated_at time.Time, updated_by int64) error {
	...
	if published && !currentPost.IsPublished {
		_, err = writeDB.Exec(stmtUpdatePostPublished, title, slug, tagline, markdown, html, featured, isPage, status, meta_description, image, updated_at, updated_by, updated_at, updated_by, id)
	} else {
		_, err = writeDB.Exec(stmtUpdatePost, title, slug, tagline, markdown, html, featured, isPage, status, meta_description, image, updated_at, updated_by, id)
	}
	if err != nil {
		writeDB.Rollback()
		return err
